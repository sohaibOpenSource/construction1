...
class ContractedOrder(models.Model):
    _name = 'sale.contracted.order'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = 'Contracted Order'
    ...
    line_ids = fields.One2many(
        'sale.contracted.order.line', 'order_id', string='Order lines',
        copy=True)
    line_count = fields.Integer(
        string='Sale Contracted Order Line count',
    ...
    def _compute_sale_count(self):
        for contracted_order in self:
            contracted_order.sale_count = len(contracted_order._get_sale_orders())
    ...
    def unlink(self):
        for order in self:
            if order.state not in ('draft', 'cancel'):
                raise UserError(_(
                    'You can not delete an open contracted order! '
    ...
    def action_confirm(self):
        ...
        for order in self:
            ...
            name = sequence_obj.next_by_code('sale.contracted.order')
            ...
            if order.sale_count > 0:
                for so in order._get_sale_orders():
                    if so.state not in ('cancel'):
                        raise UserError(_(
                            'You can not delete a contracted order with opened '
                            'purchase orders! '
    ...
    def action_view_sale_contracted_order_line(self):
        action = self.env.ref(
            'sale_contracted_order'
            '.act_open_sale_contracted_order_lines_view_tree').read()[0]
    ...
    def _search_original_uom_qty(self, operator, value):
        bo_line_obj = self.env['sale.contracted.order.line']
        res = []
...
class ContractedOrderLine(models.Model):
    _name = 'sale.contracted.order.line'
    _description = 'Contracted Order Line'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    ...
    order_id = fields.Many2one(
        'sale.contracted.order', required=True, ondelete='cascade')
    product_id = fields.Many2one(
    ...
    sale_lines = fields.One2many(
        'sale.order.line', 'contracted_order_line', string='Sale order lines',
    ...
    @api.depends(
        ...
        'sale_lines.contracted_order_line',
